...
public class AddComputerProblemChange implements ProblemChange<TransactionBalance> {

    private final LongFunction<TransactionComputer> generator;

    public AddComputerProblemChange(LongFunction<TransactionComputer> generator) {
    ...
    public void doChange(TransactionBalance cloudBalance, ProblemChangeDirector problemChangeDirector) {
        ...
        for (TransactionComputer otherComputer : cloudBalance.getComputerList()) {
            if (nextComputerId <= otherComputer.getId()) {
                nextComputerId = otherComputer.getId() + 1L;
            }
        }
        TransactionComputer computer = generator.apply(nextComputerId);
