...
public class EmvTransactionComputerPanel extends JPanel {

    private final EmvTransactionBalancingPanel transactionBalancingPanel;
    private Computer computer;
    private List<EmvProcess> processList = new ArrayList<>();
    ...
    public EmvTransactionComputerPanel(EmvTransactionBalancingPanel transactionBalancingPanel, Computer computer) {
...
//                EmvProcess::getRequiredCpuPower);
//                EmvProcess::getEstimatedExecutionTime);
                EmvProcess -> 1);//todo dummy value
        cpuPowerBar.setEnabled(false);
        add(cpuPowerBar);

        memoryBar = new CloudBar(getComputerMemory(),
                transactionBalancingPanel.getMaximumComputerMemory(),
//                EmvProcess::getRequiredMemory);
                EmvProcess -> 1);//todo dummy value
...
//                EmvProcess::getRequiredNetworkBandwidth);
                EmvProcess -> 1);//todo dummy value
    ...
    public void addProcess(EmvProcess process) {
        processList.add(process);
    }

    public void removeProcess(EmvProcess process) {
    ...
    public void update() {
        ...
        for (EmvProcess process : processList) {
...
//            usedNetworkBandwidth += process.getRequiredNetworkBandwidth();
    ...
    private class CloudBar extends JPanel {
        ...
        private ToIntFunction<EmvProcess> processValueExtractor;

        public CloudBar(int computerValue, int maximumComputerValue, ToIntFunction<EmvProcess> processValueExtractor) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            int offsetValue = 0;
            for (EmvProcess process : EmvTransactionComputerPanel.this.processList) {
    ...
    private class CloudProcessListDialog extends JDialog {
        ...
        private JPanel createAssignmentsPanel() {
            ...
            for (final EmvProcess process : processList) {
        ...
        private class CloudProcessAssignmentPanel extends JPanel {
            ...
            public CloudProcessAssignmentPanel(JPanel assignmentsPanel, EmvProcess process) {
                super(new GridLayout(0, 5));
                JPanel labelAndDeletePanel = new JPanel(new BorderLayout(5, 0));
                ...
                JTextField cpuPowerField = new JTextField(1 + " GHz");//todo dummy value instead of "process.getRequiredCpuPower()"
                cpuPowerField.setEditable(false);
                add(cpuPowerField);
                JTextField memoryField = new JTextField(1 + " GB");//todo dummy value instead of "process.getRequiredMemory()"
                memoryField.setEditable(false);
                add(memoryField);
                JTextField networkBandwidthField = new JTextField(1 + " GB");//todo dummy value instead of "process.getRequiredNetworkBandwidth()"
                networkBandwidthField.setEditable(false);
                add(networkBandwidthField);
